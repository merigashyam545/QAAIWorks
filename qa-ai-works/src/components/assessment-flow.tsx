...
export function AssessmentFlow() {
  ...
  return (
    <main className="min-h-screen bg-[#f8f9f8] text-[#082d25]">
      ...
      <section className="mx-auto max-w-5xl px-4 py-4 sm:px-5 md:px-8 md:py-6">
        <div className="border border-[#d8e0dc] bg-white">
          <div className="min-h-[calc(100vh-116px)]">
            <div className="flex min-h-[calc(100vh-116px)] flex-col p-5 sm:p-6 md:p-8 lg:px-10">
              <div className="flex items-center justify-between border-b border-[#d8e0dc] pb-3">
              ...
              <div className="mt-2 h-1 bg-[#e6ebe8]"><div className="h-1 bg-[#016b54] transition-all" style={{ width: `${leadMode ? 100 : progress}%` }} /></div>

              {!leadMode ? (
                <>
                  <div className="mt-5 flex items-center gap-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#016b54]"><span className="h-px w-6 bg-[#016b54]" /> {question.eyebrow}</div>
                  <h1 className="mt-3 max-w-3xl text-[clamp(1.8rem,4vw,3rem)] font-semibold leading-[1.04] tracking-[-0.045em]">{question.question}</h1>
                  {question.helper ? <p className="mt-3 max-w-2xl text-sm leading-5 text-[#68736d]">{question.helper}</p> : null}

                  <div className="mt-5 grid gap-2 md:grid-cols-2">
                    {question.options.map((option, index) => {
                      const active = selected === option.value;
                      return (
                        <button key={option.value} type="button" onClick={() => choose(option.value)} className={`grid min-h-[56px] w-full grid-cols-[34px_1fr_auto] items-center gap-3 border px-3 text-left transition ${active ? "border-[#016b54] bg-[#eef3ef]" : "border-[#d8e0dc] bg-white hover:border-[#9eb7ad] hover:bg-[#f7f9f7]"}`}>
                  ...
                  <div className="sticky bottom-0 mt-auto flex items-center justify-between gap-4 border-t border-[#d8e0dc] bg-white py-4">
                    <span className="text-xs text-rose-600">{error}</span>
                    <button disabled={!selected} type="button" onClick={next} className="ml-auto inline-flex items-center bg-[#041c17] px-6 py-3.5 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-35">
                ...
                <form onSubmit={submitLead}>
                  <div className="mt-6 flex items-center gap-3 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#016b54]"><span className="h-px w-6 bg-[#016b54]" /> Your analysis is ready</div>
                  <h1 className="mt-3 max-w-3xl text-[clamp(2rem,4vw,3.4rem)] font-semibold leading-[1.02] tracking-[-0.05em]">Where should we send your AI QA roadmap?</h1>
                  <p className="mt-3 max-w-2xl text-sm leading-5 text-[#68736d]">Your score is already calculated. Add your details to unlock the category breakdown, biggest gap and personalized 7-day action plan.</p>

                  <div className="mt-6 grid gap-0 border-t border-[#d8e0dc] md:grid-cols-2">
                    {[
                      ["First name", "firstName", "Your name", "text"],
                      ["Email", "email", "you@example.com", "email"],
                      ["WhatsApp number", "whatsapp", "+91 9XXXXXXXXX", "text"],
                      ["LinkedIn — optional", "linkedin", "https://www.linkedin.com/in/...", "url"],
                  ...
                  <label className="mt-5 flex max-w-3xl items-start gap-3 text-xs leading-5 text-[#5f6b65]"><input type="checkbox" checked={consent} onChange={(event) => setConsent(event.target.checked)} required className="mt-1" />I agree that QA AI Works may store my assessment and contact details to provide my result and relevant follow-up. See the <a className="font-semibold text-[#016b54] underline" href="/privacy">Privacy Policy</a>.</label>
